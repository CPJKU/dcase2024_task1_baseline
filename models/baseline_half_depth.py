...
class Network(nn.Module):
    def __init__(self, config):
        ...
        channels_per_stage = [base_channels, make_divisible(base_channels * channels_multiplier, 8)]
        self.total_block_count = 0
        self.in_c = nn.Sequential(
            Conv2dNormActivation(in_channels, channels_per_stage[0] // 4, activation_layer=torch.nn.ReLU, kernel_size=3, stride=2, inplace=False),
            Conv2dNormActivation(channels_per_stage[0] // 4, channels_per_stage[0], activation_layer=torch.nn.ReLU, kernel_size=3, stride=2, inplace=False),
        )

        self.stages = nn.Sequential()
        stage = self._make_stage(channels_per_stage[0], channels_per_stage[1], n_blocks[0], strides, expansion_rate)
        self.stages.add_module("s1", stage)
...
def get_model(n_classes=10, in_channels=1, base_channels=32, channels_multiplier=2.3, expansion_rate=3.0, n_blocks=(3,), strides=None):
    """
