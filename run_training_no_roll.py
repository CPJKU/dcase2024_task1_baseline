...
class PLModule(pl.LightningModule):
    ...
    def mel_forward(self, x):
        ...
        """
        # x = self.mel(x) #this performs resampling of waveform and computation of spectrograms
        if self.training:
            x = self.mel_augment(x)
        x = (x + 1e-5).log() # This computes the log part of the logmel spectrogram
        return x
...
if __name__ == '__main__':
    ...
    parser.add_argument('--experiment_name', type=str, default="Baseline_Ali1_sub5_tuned_localload_test")
    ...
    parser.add_argument('--channels_multiplier', type=float, default=1.8) # WS uses 1.7
    parser.add_argument('--expansion_rate', type=float, default=2.1) # WS uses 1.9
    ...
    parser.add_argument('--lr', type=float, default=0.005) # WS uses 0.001 (not the same mmacs though so take with pinch of salt)
    parser.add_argument('--warmup_steps', type=int, default=0) # default = 2000, divide by 20 for 5% subset, 10 for 10%, 4 for 25%, 2 for 50%; JS says no need to use warmup, just start
