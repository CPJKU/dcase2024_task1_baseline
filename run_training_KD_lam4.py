...
class PLModule(pl.LightningModule):
    ...
    def training_step(self, train_batch, batch_idx):
        ...
        with torch.cuda.amp.autocast():
            y_hat_soft = F.log_softmax(y_hat / self.config.temperature, dim=-1) #KLdivloss expects log-space input
            teacher_logits = teacher_logits / self.config.temperature # Need to apply temp
...
if __name__ == '__main__':
    ...
    parser.add_argument('--experiment_name', type=str, default="DCASE24_KD_PaSST2Base_Ali1_sub5_exp_sch_nolmda")
